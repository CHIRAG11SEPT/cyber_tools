...
def banner():
    ...
    print(
        color(
            "│" + " " * 13 + "LOGHAWK 2.0 - SIEM ANALYZER" + " " * 11 + "│",
            FG_CYAN,
            bold=True,
        )
    )
...
def choose_log_file() -> str:
    """Ask user for log path, with a small helper."""
...
def run_analysis(path: str):
    ...
    for key in ["ssh", "firewall", "ids", "malware", "web", "system", "ml"]:
        ...
        if count == 0:
            ...
            c = color(f"{count} alerts", FG_RED if key in ("ssh", "malware", "ids") else FG_YELLOW)
...
def view_last_report():
    ...
    if not REPORT_JSON.exists():
        print(color("[!] No report.json found. Run an analysis first.", FG_YELLOW))
        input("\nPress ENTER to return to menu...")
        return
    ...
    print(color(f"Generated at: {data.get('timestamp', 'N/A')}", FG_BLUE))
    print()

    for section, alerts in results.items():
        print(color(f"[{section.upper()}] {len(alerts)} alert(s)", FG_CYAN if alerts else FG_GREEN))
    print()
    print(color("Open report.json or report.txt for full details.", FG_YELLOW))
    input("\nPress ENTER to return to menu...")
...
def main_menu():
    while True:
        ...
        print()
        choice = input(color("Select an option (1-3): ", FG_WHITE, bold=True)).strip()
        ...
        if choice == "1":
            ...
            clear_screen()
            print(color("Goodbye! Stay secure. 🛡️", FG_GREEN, bold=True))
            break
        else:
